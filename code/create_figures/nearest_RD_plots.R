...
plot_PRD <- subset(
  ...
  abs(nearest_neighbor_PRD$PRD_margin) < bw$bws[1]
)
...
plot_PRD_bins <- plot_PRD %>%
  ...
  group_by(percentile) %>%
  summarise(
    avg_change_pp = mean(change_pct_PRD, na.rm = TRUE),
...
PRD_RD <- ggplot(plot_PRD, aes(x = PRD_margin, y = change_pct_PRD)) +
  geom_point(
    aes(x = bin_center, y = avg_change_pp),
    alpha = 0.5,
    size = 3,
    data = subset(plot_PRD_bins, bin_center < 0)
  ) +
  geom_point(
    aes(x = bin_center, y = avg_change_pp),
    alpha = 0.5,
  ...
  geom_smooth(
    method = "lm",
    data = subset(plot_PRD, PRD_margin < 0),
    color = "black",
    level = 0.9
  ) +
  geom_smooth(
  ...
  labs(title = "", x = "PRD Vote Margin, t1", y = "Change in PRD vote share, t2") +
  theme_minimal()

print(PRD_RD)

ggsave(
  filename = "C:/Users/adamd/Dropbox/Apps/Overleaf/TYP/images/PRD_RD.pdf",
  plot = PRD_RD,
  width = 6,
  height = 4
)
...
plot_PAN <- subset(
  nearest_neighbor_PAN,
  abs(nearest_neighbor_PAN$PAN_margin) < bw$bws[1]
)
...
plot_PAN_bins <- plot_PAN %>%
  ...
  group_by(percentile) %>%
  summarise(
    avg_change_pp = mean(change_pct_PAN, na.rm = TRUE),
...
PAN_RD <- ggplot(plot_PAN, aes(x = PAN_margin, y = change_pct_PAN)) +
  geom_point(
    ...
    alpha = 0.5,
    size = 3,
    data = subset(plot_PAN_bins, bin_center < 0),
  ) +
  geom_point(
    aes(x = bin_center, y = avg_change_pp),
    alpha = 0.5,
  ...
  geom_smooth(
    method = "lm",
    color = "black",
    data = subset(plot_PAN, PAN_margin < 0),
    level = 0.9
  ) +
  geom_smooth(
    method = "lm",
...
ggsave(
  filename = "C:/Users/adamd/Dropbox/Apps/Overleaf/TYP/images/PAN_RD.pdf",
  plot = PAN_RD,
  width = 6,
  height = 4
)
