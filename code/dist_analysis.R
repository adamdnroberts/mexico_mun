...
neighbors2 <- merge(vec5,neighbors_PAN, by.x = c("neighbor"), by.y = c("mun_id"))
...
help <- subset(df, select = c(mun_id,year,PAN_pct))
help2 <- subset(avd, select = c(mun_id, main_year, PAN_pct))

melp <- merge(help, help2, by.x = c("mun_id", "year"), by.y = c("mun_id","main_year"))

DCdensity(melp$PAN_pct.x, cutpoint = 0.5)
DCdensity(melp$PAN_pct.y, cutpoint = 0.5)
