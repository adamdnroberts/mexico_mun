...
nearest_neighbor_PAN <- rdd_PAN_subset %>%
  filter(ref_PAN_wins == 0, main_estado == ref_estado) %>%
  group_by(mun_id) %>%
  slice_head(n = 1) %>%
  ungroup() %>%
  mutate(
...
nearest_untreated_distances <- nearest_neighbor_PAN %>%
