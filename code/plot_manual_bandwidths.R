...
run_rd_models <- function(data, y, x, bandwidths_pct, confidence_level) {
  ...
  results <- data.frame(
    ...
    Coefficient = sapply(models, function(m) m$coef[1]),
    CI_Lower = sapply(models, function(m) m$ci[1, 1]),
    CI_Upper = sapply(models, function(m) m$ci[1, 2])
  )
...
results_90 <- run_rd_models(PRD_nn, PRD_nn$change_pp_PRD, PRD_nn$PRD_pct, bandwidths_pct, 90)
names(results_90)[3:4] <- c("CI_Lower90", "CI_Upper90")

# Run models with 95% confidence intervals
results_95 <- run_rd_models(PRD_nn, PRD_nn$change_pp_PRD, PRD_nn$PRD_pct, bandwidths_pct, 95)
...
# Run models with 90% confidence intervals
results_90 <- run_rd_models(PAN_nn, PAN_nn$change_pp_PAN, PAN_nn$PAN_pct, bandwidths_pct, 90)
names(results_90)[3:4] <- c("CI_Lower90", "CI_Upper90")

# Run models with 95% confidence intervals
results_95 <- run_rd_models(PAN_nn, PAN_nn$change_pp_PAN, PAN_nn$PAN_pct, bandwidths_pct, 95)
names(results_95)[3:4] <- c("CI_Lower95", "CI_Upper95")
